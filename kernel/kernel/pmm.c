...
void init_pmm(multiboot_info_t* mbd){
	...
	while(entry < mbd->mmap_addr + mbd->mmap_length){
		...
		if(type == MULTIBOOT_MEMORY_AVAILABLE){
			...
			for(uint64_t addrOffset = 0; addrOffset < len; addrOffset += MEMORY_BLOCK_SIZE){
				if(len-addrOffset < MEMORY_BLOCK_SIZE)
					break;
				free_addrs_size++;
				free_addrs[workingI].addr = addr+addrOffset;
				free_addrs[workingI++].len = MEMORY_BLOCK_SIZE;
			}
		}
	...
	for(uint64_t i = 0; i < free_addrs_size; i++){
		...
		if(curr.len != 0){
			...
			for(uint64_t j = i+1; j < i+size; j++){
				...
				if(temp->addr != lastBlock.addr+MEMORY_BLOCK_SIZE || temp->len == 0){
					found = j;
					break;
				}
			}
